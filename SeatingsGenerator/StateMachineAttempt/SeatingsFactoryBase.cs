namespace ConsoleApplication5
{
    ...
    public abstract class SeatingsFactoryBase : ISeatingsFactory
    {
...
#if STATE_MACHINE
        public State[] GetSeatingsParallel()
#else
...
#endif
        {
            ...
            int[] players;
            int i;
            int[][] arraySeatings = GetArraySeatings(MaxParallelSize, out players, out i);
            ...
            {
#if STATE_MACHINE
                var ienum = new State { Seating = seating, Players = players.Skip(i).ToArray() };
#else
...
#if STATE_MACHINE
        public abstract bool MoveNextSeating(State state);
#endif
            ...
            if (PodSizes[p] == 5)
            {
                if (first != 0)
                {
                    ...
                    if (third != 0 && InitialRelationShip[firstIndex + third] == RelationShip.GrandPredator
                        || fourth != 0 && InitialRelationShip[firstIndex + fourth] == RelationShip.GrandPrey)
                    {
                        return true;
                    }
                }
                ...
                if (second != 0)
                {
                    ...
                    if (fourth != 0 && InitialRelationShip[secondIndex + fourth] == RelationShip.GrandPredator
                        || fifth != 0 && InitialRelationShip[secondIndex + fifth] == RelationShip.GrandPrey)
                    {
                        return true;
                    }
                }
...
#if STATE_MACHINE
        public IEnumerable<int[]> GetBestSeatings(State state)
#else
